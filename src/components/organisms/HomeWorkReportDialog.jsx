...
export default function HomeworkReportDialog({ open, onClose }) {
    ...
    const handleGenerate = () => {
        // Petición al backend con filtros (mock data)
        setReportData([
            {
                fecha: "2025-05-10",
                nombre: "Ana Gómez",
    ...
    const handleExportPDF = () => {
        console.log("Exportar PDF de tareas");
    };

    const handleExportExcel = () => {
        console.log("Exportar Excel de tareas");
    };
                            ...
                            <table className="min-w-full border border-gray-200 text-sm">
                                <thead className="bg-gray-100 text-left">
                                    <tr>
                                        ...
                                        <th className="p-2 border">Porcentaje Entregado</th>
                                        <th className="p-2 border">Promedio de Calificación</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {reportData.map((row, i) => (
                                        <tr key={i} className="hover:bg-gray-50">
                                            ...
                                            <td className="p-2 border">{row.estado}</td>
                                            <td className="p-2 border">{row.porcentaje}</td>
                                            <td className="p-2 border">{row.promedio}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
